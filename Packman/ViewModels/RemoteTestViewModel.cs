...
public sealed class RemoteTestViewModel : ObservableObject
{
    ...
    // ── Package under test ─────────────────────────────────────────────
    /// <summary>
    /// The package staged on the target. Pre-filled with the package the wizard just
    /// generated; when Remote Test is opened from the rail there is none, so the user
    /// browses to one on the share.
    /// </summary>
    ...
    /// <summary>
    /// Takes a package built earlier, reading its name and version back from the PSADT
    /// script. There is no installer to read metadata from, so detection discovery works
    /// from the script values alone.
    /// </summary>
